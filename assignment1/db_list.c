...
{
    struct db_list* dbl = malloc(sizeof(struct db_list));
    dbl->head = NULL;
    dbl->tail = NULL;
    return dbl;    
}
...
void db_list_free(struct db_list* db_list)
{
    ...
    while(current != NULL){
        struct db_node* next_node = current->next;
        free(current);
        current = next_node;        
    }  
    free(db_list);
...
{
	/* Hint: use the 'tail' pointer instead of iterating through the whole list */
    if(db_list == NULL || db_list->head == NULL){
        return;
    }
	struct db_node* to_remove = db_list -> tail;
